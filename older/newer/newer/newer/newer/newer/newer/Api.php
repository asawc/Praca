<?php 
	...
	if(isset($_GET['apicall'])){
		...
		switch($_GET['apicall']){
			...
			case 'add_release':
				if(isTheseParametersAvailable(array('id_employee', 'surname', 'name'))){
					$id_employee=$_POST['id_employee'];
					$id_product=$_POST['surname'];
					$id_quantity=$_POST['name'];
					
					$query="INSERT INTO ". RELEASES_TABLE .
							" (". RELEASES_ID_EMPLOYEE .", ". RELEASES_EMPLOYEE_SURNAME .",  ". RELEASES_EMPLOYEE_NAME .") VALUES (?, ?, ?)";
					$stmt = $conn->prepare($query);
					$stmt->bind_param("iss", $id_employee, $surname, $name);
