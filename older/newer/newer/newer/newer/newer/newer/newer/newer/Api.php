					...
					if(isset($_GET['id'])){
						...
						if($stmt->num_rows > 0){
							...
							//	$stmt->close();
								...
								$release= new Release($rel[RELEASES_ID], $employee->toAssocArray() , $rel['status'], 
								$rel['date_creation'], $rel['date_realizing'], $productsRelease);
								
								// print_r($release);
								$response['error'] = false; 
								$response['message'] = 'Release fetched successfully'; 
								$response['release'] = $release->toAssocArray(); 
							} else {
								$release['productsRelease']=null;
